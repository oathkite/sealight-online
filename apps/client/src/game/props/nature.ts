...
export const buildOak = (s: Shapes, size = 1): PropAnchors => {
  ...
  s.ball([0, 2.0 * k, 0], [0.95 * k, 0.8 * k, 0.95 * k], "leaf", { material: MATERIAL.foliage });
  ([[0.55, 1.75, 0.35], [-0.6, 1.8, 0.15], [0.1, 1.8, -0.6], [0.1, 2.4, 0.15]] as const).forEach(([x, y, z], i) =>
    s.ball([x * k, y * k, z * k], [0.55 * k, 0.5 * k, 0.55 * k], i % 2 ? "leaf" : "leaf_light", { material: MATERIAL.foliage }));
  ([[0.45, 1.6, 0.78], [-0.4, 1.75, 0.8], [0.82, 2.0, 0.3]] as const).forEach(([x, y, z]) =>
    s.ball([x * k, y * k, z * k], [0.09 * k, 0.09 * k, 0.09 * k], "apple", { material: MATERIAL.gloss }));
  return NONE;
};
...
export const buildPine = (s: Shapes, size = 1): PropAnchors => {
  ...
  ([[0.5, 0.95, 1.25], [1.15, 0.72, 1.05], [1.72, 0.5, 0.9]] as const).forEach(([y, r, h]) =>
    s.cylinder([0, y * k, 0], r * k, 0, h * k, "pine", { material: MATERIAL.foliage, round: 0.14 * k }));
  return NONE;
};

/** 茂み。小さな花がのぞく */
export const buildBush = (s: Shapes): PropAnchors => {
  ([[0, 0, 0.45], [0.32, 0.12, 0.33], [-0.28, 0.16, 0.3]] as const).forEach(([x, z, r]) => s.ball([x, r * 0.72, z], [r, r * 0.82, r], "leaf", { material: MATERIAL.foliage }));
  const rng = createRng(41);
  for (let i = 0; i < 6; i += 1) {
    const a = rng.next() * Math.PI * 2;
    s.ball([Math.cos(a) * 0.32, 0.25 + rng.next() * 0.25, Math.sin(a) * 0.3 + 0.12], [0.055, 0.05, 0.055], "flower_lavender");
  }
  return NONE;
};
...
export const buildPondEdge = (s: Shapes): PropAnchors => {
  ...
  ([[-1.55, -0.7, 0.62], [-1.65, -0.4, 0.48], [1.6, 0.55, 0.52], [1.5, 0.78, 0.42], [-1.2, 0.95, 0.4]] as const).forEach(([x, z, h]) =>
    s.cylinder([x, -0.08, z], 0.035, 0, h, "reed", { round: 0.012, material: MATERIAL.foliage }));
...
export const buildFlower = (s: Shapes, pick: number, size: number): void => {
  ...
  s.cylinder([0, 0, 0], 0.016, 0.013, 0.17 * size, "reed", { round: 0.006 });
  for (let k = 0; k < 5; k += 1) {
    const a = (k / 5) * Math.PI * 2 + pick * 3;
    s.ball([Math.cos(a) * 0.05 * size, 0.18 * size, Math.sin(a) * 0.05 * size], [0.045 * size, 0.02 * size, 0.045 * size], petal);
  }
  s.ball([0, 0.19 * size, 0], [0.028 * size, 0.022 * size, 0.028 * size], petal === "flower_yellow" ? "flower_white" : "flower_yellow");
};
